...
class Critic(nn.Module):
    ...
    def __init__(self, numberOfInputs, numberOfOutputs, numberOfNeurons=numberOfNeurons, dropout=dropout):
        ...
        self.fc9 = nn.Linear(numberOfNeurons, numberOfNeurons)
        self.fc0 = nn.Linear(numberOfNeurons, 1)

        # Definition of some Batch Normalization layers
        self.bn6 = nn.BatchNorm1d(numberOfNeurons)
        self.bn7 = nn.BatchNorm1d(numberOfNeurons)
        self.bn8 = nn.BatchNorm1d(numberOfNeurons)
        self.bn9 = nn.BatchNorm1d(numberOfNeurons)
    ...
    def forward(self, input, actions):
        ...
        x = self.dropout9(F.leaky_relu(self.bn9(self.fc9(x))))
        q2 = self.fc0(x)
    ...
    def learning(self, batchSize=batchSize):
        ...
        if (len(self.replayMemory) >= batchSize):
            ...
            with torch.no_grad():
                ...
                noise = (torch.randn_like(action) * 0.1).clamp(0, 1)
                nextActions = (self.pi_target(nextState) + noise).clamp(0, 1)
            ...
            #    _, nextActions = self.pi(nextState).max(1)
            #    nextQValues = self.q_target(nextState, nextActions.unsqueeze(1)).squeeze(1)
            #    expectedQValues = reward + gamma * nextQValues * (1 - done)
            ...
            # Compute the Policy loss by Delayed policy update
