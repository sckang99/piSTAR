...
class ReplayMemory:
    ...
    def sample(self, batchSize):
        ...
        state, action, reward, prob, nextState, done = zip(*random.sample(self.memory, batchSize))
        return state, action, reward, prob, nextState, done
...
#            self.pi_target.load_state_dict(self.pi.state_dict())
    ...
    def chooseAction(self, state):
        ...
        with torch.no_grad():
            ...
            prob = self.pi(tensorState).cpu()
            m = Categorical(prob)
            action = m.sample().item()
            return action, prob
    
    def chooseActionEpsilonGreedy(self, state):
        """
        GOAL: Choose a valid RL action from the action space according to the
        ...
        action, prob_a = self.chooseAction(state)
    ...
    def learning(self, batchSize=batchSize):
        ...
        if (len(self.replayMemory) >= batchSize):
            ...
            state, action, reward, prob_a, nextState, done = self.replayMemory.sample(batchSize)
            ...
            prob_a = torch.tensor(prob_a, dtype=torch.float, device=self.device)
            ...
            v_cur = self.v(state)
            v_next = self.v(nextState)

            # generate advacend estimation by GAE
            advs = torch.zeros_like(reward, device=self.device)
            future_ages = torch.tensor(0.0, dtype = reward.dtype, device=self.device)
            future_advs = v_next[-1]
            
            for t in reversed(range(batchSize)):
                advs[t] = future_advs = reward[t] + gamma * future_advs * (1 - done)
            ...
            value_loss = F.smooth_l1_loss(v_cur, v_target)
            ...
            pi_cur = self.pi(state).clamp(0, 1)
            pi_a = pi_cur.gather(1, action.to(torch.long))
            ...
            for episode in tqdm(range(trainingParameters[0]), disable=not(verbose)):                
                ...
                for i in range(len(trainingEnvList)):
                    ...
                    while done == 0:
                        ...
                        action, prob_a = self.chooseActionEpsilonGreedy(state)
                        ...
                        self.replayMemory.push(state, action, reward, prob_a[action], nextState, done)
                        ...
                        otherDone = info['Done']
                        self.replayMemory.push(state, otherAction, otherReward, prob_a[otherAction], otherNextState, otherDone)
    ...
    def testing(self, trainingEnv, testingEnv, rendering=False, showPerformance=False):
        ...
        while done == 0:
            ...
            # Choose an action according to the RL policy and the current RL state
            action, _ = self.chooseAction(state)
            ...
            v = self.v(tensorState.unsqueeze(0))
            # Interact with the environment with the chosen action
        ...
        if rendering:
            testingEnv.render()
            self.plotQValues(VValues, VValues, testingEnv.marketSymbol)
        ...
        try:
            ...
            for iteration in range(iterations):
                ...
                for episode in tqdm(range(trainingParameters[0])):
                    ...
                    for i in range(len(trainingEnvList)):
                        ...
                        while done == 0:
                            ...
                            # Choose an action according to the RL policy and the current RL state
                            action, prob_a = self.chooseActionEpsilonGreedy(state, previousAction)
                ...
                if iteration < (iterations-1):
                    trainingEnv.reset()
                    testingEnv.reset()
                    self.v.load_state_dict(qinitialWeights)
    ...
    def saveModel(self, vfileName, pfileName):
        """
        ...
        torch.save(self.v.state_dict(), vfileName)
        torch.save(self.pi.state_dict(), pfileName)
        


    def loadModel(self, vfileName, pfileName):
        """
        ...
        self.v.load_state_dict(torch.load(vfileName, map_location=self.device))
