...
n_step_return = 11    # set this 0 for GAE
val_loss_coef = 0.5
batch_size    = 128
...
def train_net(model, memory):
    ...
    for _ in range(4):
        ...
        pi_a = pi.gather(1,a.to(torch.long))  # need int64

        policy_loss = -torch.log(pi_a) * advs.detach()
        val_loss = val_loss_coef * F.mse_loss(v, v_target.detach())  # you need the detach() for one of them
...
def train(window_size=20, starting_balance = 1000000, resume_epoch=0, max_epoch=1000):  
    ...
    plt.savefig('A2C_S00001_train.png')
    plt.show()
    plt.pause(3)
    plt.close()
